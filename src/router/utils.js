class Utils {
    ...
    isEmpty(variable) {
        return (
            this.isUndefined(variable) ||
            this.isNull(variable) ||
            variable === 0 ||
            variable === false ||
            ((this.isString(variable) || this.isArray(variable)) && variable.length === 0) ||
            (this.isObject(variable) && Object.getOwnPropertyNames(variable).length === 0)
        );
    }
...
export function ArgumentNotFoundError(argName) {
    ...
    let message = Utils.isSet(argName)
        ? `${argName} argument is required. None found`
...
export function ArgumentTypeError(argName, argType, argValue) {
    ...
    let message = Utils.isSet(argName)
        ? `typeof ${argName.toString()} argument must be equal to ${argType}. ${typeof argValue} found.`
