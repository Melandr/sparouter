import { Utils, ArgumentNotFoundError as ArgNotFound, ArgumentTypeError as ArgTypeError, QueryParams } from "./utils";
...
class SPARouter {
     ...
     * }, {argument: "A stored argument from my callback function"}); // this outputs "a stored argument from my callback function" to the console.
     *
    ...
    where(name, regExp) {
        ...
        if (route.parameters.length === 0)
            throw new Error(
                `No Parameters Found: Could not set paramater regExpression for [${route.uri}] because the route has no parameters`
            );
        ...
        if (!parameterFound)
            throw new Error(
                `Invalid Parameter: Could not set paramater regExpression for [${route.uri}] because the parameter [${name}] does not exist`
            );
            ...
            if (route.name === name) {
                ...
                if (this._containsParameter(uri)) {
                    ...
                    if (array.length !== Object.getOwnPropertyNames(parameters).length)
                        throw new Error(
                            `The route with name [${name}] contains ${array.length} parameters. ${
                                Object.getOwnPropertyNames(parameters).length
                            } given`
                        );
